...
public class GeneratePath : MonoBehaviour
{
    ...
    private void GenerateNewPath()
    {
        ...
        transform.position = new Vector3((int) maxWidth/2, yoffset, 0);
        
        int length = 0;
        
        while(length+1 < pathLength)
        {
            ...
            if (Mathf.Abs(transform.position.x + direction.x) < maxWidth)
            {
                //Check if the tile is already placed
                if((int)transform.position.x + direction.x < 0 || (int)transform.position.x + direction.x >= maxWidth)
                    continue;
